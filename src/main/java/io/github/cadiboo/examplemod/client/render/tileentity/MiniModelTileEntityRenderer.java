...
public class MiniModelTileEntityRenderer extends TileEntityRenderer<MiniModelTileEntity> {
	...
	public void render(final MiniModelTileEntity tileEntityIn, final float partialTicks, final MatrixStack matrixStack, final IRenderTypeBuffer renderTypeBuffer, final int packedLight, final int backupPackedLight) {
		...
		final float scale = ExampleModConfig.modelScale;
		...
		// Render the buffers
		renderChunkBuffers(miniModel.regionRenderCacheBuilder, miniModel.getCompiledChunk(), renderTypeBuffer);
	 ...
	 */
	private void renderChunkBuffers(final RegionRenderCacheBuilder buffers, final CompiledChunk compiledChunk, final IRenderTypeBuffer renderTypeBuffer) {
		// Render each buffer that has been used
		for (final RenderType renderType : RenderType.getBlockRenderTypes())
			if (!compiledChunk.isLayerEmpty(renderType)) {
				final IRenderTypeBuffer.Impl impl = (OutlineLayerBuffer.Impl) renderTypeBuffer;
				final BufferBuilder bufferBuilder = buffers.getBuilder(renderType);
				final IVertexBuilder implBuffer = impl.getBuffer(renderType);
				{
					// Get the internal data from the BufferBuilder (This resets the BufferBuilder's own copy of this data)
					final Pair<BufferBuilder.DrawState, ByteBuffer> pair = bufferBuilder.getAndResetData();
					final ByteBuffer byteBuffer = pair.getSecond();
					...
					// TODO: FIXME: This doesn't work
					((BufferBuilder) implBuffer).putBulkData(byteBuffer);
				}
			}
	}
